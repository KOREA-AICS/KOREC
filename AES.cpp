...
// FIPS-197 표준을 준수하며 T-Box 최적화 기법을 적용한 AES 클래스
class AES_Optimized {
private:
    // --- 상수 테이블 (FIPS-197 표준) ---
    ...
    // 바이트 배열을 uint32_t로 변환 (Big Endian)
    ...
    // uint32_t를 바이트 배열로 변환 (Big Endian)
...
public:
    ...
    void decrypt_block(const uint8_t* ciphertext, uint8_t* plaintext) {
        uint32_t s0 = bytes_to_uint32(ciphertext)      ^ round_keys[round_keys.size() - 4];
        uint32_t s1 = bytes_to_uint32(ciphertext + 4)  ^ round_keys[round_keys.size() - 3];
        uint32_t s2 = bytes_to_uint32(ciphertext + 8)  ^ round_keys[round_keys.size() - 2];
        uint32_t s3 = bytes_to_uint32(ciphertext + 12) ^ round_keys[round_keys.size() - 1];

        int Nr = round_keys.size() / 4 - 1;

        for (int round = Nr - 1; round >= 1; --round) {
            uint32_t t0 = Td0[(s0 >> 24) & 0xFF] ^ Td1[(s3 >> 16) & 0xFF] ^ Td2[(s2 >> 8) & 0xFF] ^ Td3[s1 & 0xFF] ^ round_keys[round*4];
            uint32_t t1 = Td0[(s1 >> 24) & 0xFF] ^ Td1[(s0 >> 16) & 0xFF] ^ Td2[(s3 >> 8) & 0xFF] ^ Td3[s2 & 0xFF] ^ round_keys[round*4+1];
            uint32_t t2 = Td0[(s2 >> 24) & 0xFF] ^ Td1[(s1 >> 16) & 0xFF] ^ Td2[(s0 >> 8) & 0xFF] ^ Td3[s3 & 0xFF] ^ round_keys[round*4+2];
            uint32_t t3 = Td0[(s3 >> 24) & 0xFF] ^ Td1[(s2 >> 16) & 0xFF] ^ Td2[(s1 >> 8) & 0xFF] ^ Td3[s0 & 0xFF] ^ round_keys[round*4+3];
             s0 = t0; s1 = t1; s2 = t2; s3 = t3;
        }
